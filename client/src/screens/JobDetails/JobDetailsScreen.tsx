...
export default function JobDetailsScreen({
  ...
  useEffect(() => {
    ...
    fetchTemplate({
      ...
      onFail: (msg) =>
        setError(
          `Unable to load template configuration (${jobConfig.templateId}): ${msg}`
        ),
    });
  ...
  useEffect(() => {
    ...
    fetchWorkspace({
      ...
      onFail: (msg) =>
        setError(
          `Unable to load workspace configuration (${jobConfig.workspaceId}): ${msg}`
        ),
    });
  ...
  useEffect(() => {
    ...
    fetchJobInfo({
      ...
      onFail: (msg) =>
        setError(
          `Unable to load job report information (${jobConfig.latestExec}): ${msg}`
        ),
    });
            ...
            <div className="flex justify-between items-center relative w-full my-4">
              <div className="flex flex-row space-x-2 items-center">
                ...
                <Select
                  ...
                  onChange={(event) =>
                    setFilter(
                      event.target.value ? event.target.value === "true" : null
                    )
                  }
                >
              ...
              <div className="table w-full p-1 pb-2">
                <Table hoverable>
                  ...
                  <Table.Body className="divide-y">
                    {records
                      ?.filter(
                        (record) => filter === null || record.success === filter
                      )
                      .filter(
                        (record) =>
                          searchFor === null ||
                        ...
                        switch (sortBy) {
                          ...
                          case "status":
                            return (t(formatRecordStatus(a)).toLowerCase() ??
                              0) < (t(formatRecordStatus(b)).toLowerCase() ?? 0)
                              ? -1
                              : 1;
                          default:
                            return 1;
