...
function MultiRowInput({
  ...
  inputConfiguration,
  onChange,
}: Pick<FindAndReplaceLiteralOperationInputProps, "inputConfiguration"> & {
  items: { literal: string; value: string }[];
  onChange: React.Dispatch<
    React.SetStateAction<{ literal: string; value: string }[]>
  >;
}) {
      ...
      {items.map((item, index) => (
        ...
        >
          ...
          {inputConfiguration && inputConfiguration.type === "textarea" && (
            <Textarea
              id={`replace-literal-textareainput-${index}`}
              className="resize-none"
              value={item.literal}
              rows={4}
              placeholder={t("alter Wert")}
              color={getTextInputColor({
                ok:
                  focus === `replace-literal-textareainput-${index}`
                    ? null
                    : item.literal !== "",
              })}
              onChange={(e) =>
          ...
          {(!inputConfiguration ||
            inputConfiguration.type === "text" ||
            inputConfiguration.type === "select") && (
            <TextInput
              id={`replace-literal-textinput-${index}`}
              className="w-1/2"
              type="text"
              sizing={"sm"}
              placeholder={t("alter Wert")}
              ...
              color={getTextInputColor({
                ok:
                  focus === `replace-literal-textinput-${index}`
                    ? null
                    : item.literal !== "",
              })}
          ...
          {inputConfiguration && inputConfiguration.type === "textarea" && (
            <Textarea
              ...
              rows={4}
              // this is part of the application's configuration > no translation
              placeholder={inputConfiguration?.placeholder || t("neuer Wert")}
              color={getTextInputColor({
                ok:
...
interface FindAndReplaceLiteralOperationInputProps {
  targetField: string;
...
export default function FindAndReplaceLiteralOperationInput({
  targetField,
  operation,
  inputConfiguration,
  onChange,
}: FindAndReplaceLiteralOperationInputProps) {
  const [items, setItems] = useState<{ literal: string; value: string }[]>(
    // fallback should not be relevant since Input is expected to be
    // created with at least one value
    operation.items || [{ literal: "", value: "" }]
  );
  ...
  * runs again on every change in the input
  ...
  useEffect(() => {
    if (items.length !== 0) {
      ...
      */
      const newOperation: FindAndReplaceLiteralOperation = {
      ...
      onChange((prev) => [
        ...prev.filter(
          (operation) =>
            operation.type !== "findAndReplaceLiteral" ||
  ...
  return (
    <div className="flex flex-col items-center w-full space-y-2 my-2">
      <div className="flex flex-row w-full text-sm items-center justify-between">
        {t("Gezielt Werte in Feldern ersetzen")}
      </div>
